...
public partial class AdminDashboardPage : ContentPage
{
    ...
    private async Task LoadStatsAsync()
    {
        try
        {
            var products = await FirebaseAuthService.Instance.GetAllProductsAsync();
            var categories = await FirebaseAuthService.Instance.GetCategoriesAsync();
            var orders = await FirebaseAuthService.Instance.GetAllOrdersAsync();
