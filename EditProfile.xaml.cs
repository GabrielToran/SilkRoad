...
public partial class EditProfile : ContentPage
{
    ...
    {
        string? userId = FirebaseService.Instance.CurrentUserId;
        if (userId == null) return;

        _profile = await FirebaseService.Instance.GetUserProfileAsync(userId);
    ...
    private async void OnSaveClicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            await FirebaseService.Instance.UpdateUserProfileAsync(_profile);
    ...
    private async void OnChangePasswordTapped(object sender, TappedEventArgs e)
    {
        ...
        if (confirm && !string.IsNullOrEmpty(email))
        {
            try
            {
                await FirebaseService.Instance.SendPasswordResetAsync(email);
